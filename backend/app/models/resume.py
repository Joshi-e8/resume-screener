...
class UploadedResume(Document):
    ...
    status: str = Field(default="UPLOADED", pattern="^(UPLOADED|PROCESSING|PARSED|ERROR)$")
...
class ResumeAnalysis(Document):
    ...
    status: str = Field(default="PENDING", pattern="^(PENDING|ANALYZED|SHORTLISTED|REJECTED)$")
    created_at: datetime = Field(default_factory=lambda: datetime.now(timezone.utc))
