...
async def login(login_data: LoginRequest) -> Any:
    ...
    is_authenticated = await user_service.authenticate_user(login_data.email, login_data.password)
...
async def verify_otp(user_id: str, otp_payload: OtpPayload) -> Any:
    ...
    try:
        ...
        if not stored_otp:
            return JSONResponse(
                status_code=status.HTTP_400_BAD_REQUEST,
                content={"result": "failure", "errors": {"otp": "OTP expired. Please request a new one."}}
            )

        if stored_otp.decode() != otp_payload.otp:
            return JSONResponse(
                status_code=status.HTTP_400_BAD_REQUEST,
                content={"result": "failure", "errors": {"otp": "The entered OTP is incorrect. Please try again."}}
            )   
