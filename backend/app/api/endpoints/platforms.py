...
async def connect_platform(
    platform_id: str, auth_data: dict, current_user: User = Depends(get_current_user)
) -> Any:
    """
    ...
    try:
        connection = await platform_service.connect_platform(
            user_id=str(current_user.id), platform_id=platform_id, auth_data=auth_data
        )
        return {
...
async def disconnect_platform(
    platform_id: str, current_user: User = Depends(get_current_user)
) -> Any:
    """
...
async def post_job_to_platforms(
    posting_request: JobPostingRequest, current_user: User = Depends(get_current_user)
) -> Any:
    """
...
async def get_platform_analytics(
    platform_id: str, days: int = 30, current_user: User = Depends(get_current_user)
) -> Any:
    """
