...
from app.models.analytics import AnalyticsEvent, DailyMetrics, PlatformMetrics, EventType
...
class AnalyticsService:
    ...
    async def get_dashboard_stats(self, user_id: str) -> Dict:
        ...
        platform_posts_today = len([
            e for e in today_events 
            if e.event_type == EventType.JOB_POSTED
        ])
    ...
    async def get_platform_performance(
        ...
        for metric in platform_metrics:
            ...
            platform_data[platform_id]["applications_received"] += metric.applications_received
            ...
            if metric.cost_per_post:
                platform_data[platform_id]["total_cost"] += metric.cost_per_post * metric.jobs_posted
        
        # Format response
        result = []
        for platform_id, data in platform_data.items():
            total_posts = data["successful_posts"] + data["failed_posts"]
            success_rate = (data["successful_posts"] / total_posts * 100) if total_posts > 0 else 0
    ...
    async def get_resume_analytics(
        ...
        uploaded_count = len([e for e in events if e.event_type == EventType.RESUME_UPLOADED])
        processed_count = len([e for e in events if e.event_type == EventType.RESUME_PROCESSED])
        ai_analyzed_count = len([e for e in events if e.event_type == EventType.AI_ANALYSIS_COMPLETED])
        ...
        return {
            ...
            "processing_success_rate": (processed_count / uploaded_count * 100) if uploaded_count > 0 else 0
        }
    ...
    async def get_job_analytics(
        ...
        applications_received = len([e for e in events if e.event_type == EventType.APPLICATION_RECEIVED])
        
        return {
            "jobs_created": jobs_created,
            "jobs_posted": jobs_posted,
            "applications_received": applications_received,
            "avg_applications_per_job": applications_received / jobs_posted if jobs_posted > 0 else 0
        }
    ...
    async def get_ai_performance(
        ...
        ai_analyses = [e for e in events if e.event_type == EventType.AI_ANALYSIS_COMPLETED]
        candidate_matches = [e for e in events if e.event_type == EventType.CANDIDATE_MATCHED]
        
        # Calculate average processing time for AI
        ai_processing_times = [e.processing_time_ms for e in ai_analyses if e.processing_time_ms]
        avg_ai_processing_time = sum(ai_processing_times) / len(ai_processing_times) if ai_processing_times else 0
        
        # Calculate success rate
        successful_analyses = len([e for e in ai_analyses if e.success])
        ai_success_rate = (successful_analyses / len(ai_analyses) * 100) if ai_analyses else 0
    ...
    async def get_trends(
        ...
        while current_date <= end_date:
            ...
            if metric == "resumes":
                value = len([e for e in day_events if e.event_type == EventType.RESUME_PROCESSED])
            elif metric == "jobs":
                value = len([e for e in day_events if e.event_type == EventType.JOB_POSTED])
            elif metric == "matches":
                value = len([e for e in day_events if e.event_type == EventType.CANDIDATE_MATCHED])
            else:
                value = 0
        ...
        if len(data_points) >= 2:
            ...
            percentage_change = (total_change / first_value * 100) if first_value > 0 else 0
        else:
            total_change = 0
            percentage_change = 0
    ...
    async def generate_summary_report(
        ...
        platform_performance = await self.get_platform_performance(user_id, start_date, end_date)
        resume_analytics = await self.get_resume_analytics(user_id, start_date, end_date)
