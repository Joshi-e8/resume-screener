...
class AIAnalyzer:
    ...
    async def analyze_resume_comprehensive(
        ...
        job_match = None
        if job_context:
            job_match = await self.calculate_job_compatibility(
                parsed_data, job_context
            )
    ...
    def _calculate_compatibility_metrics(
        ...
        experience_match = self._match_experience_level(candidate_experience, required_experience)
        
        # Education matching
        candidate_education = parsed_resume.get('education', [])
        required_education = job_context.get('education_level', '')
        
        education_match = self._match_education_level(candidate_education, required_education)
        ...
        location_match = self._match_location(candidate_location, job_location, remote_allowed)
        ...
        return {
            ...
            "overall_compatibility": round(
                (skill_match_percentage + experience_match + education_match + location_match) / 4, 1
            )
        }
    ...
    def _match_education_level(self, candidate_education: List[Dict], required_level: str) -> float:
    ...
    def _match_location(self, candidate_location: str, job_location: str, remote_allowed: bool) -> float:
    ...
    def _combine_scores(self, ai_analysis: Dict, compatibility_metrics: Dict) -> Dict[str, float]:
    ...
    async def batch_analyze_resumes(
        ...
        for file_path in file_paths:
            try:
                analysis = await self.analyze_resume_comprehensive(file_path, job_context)
