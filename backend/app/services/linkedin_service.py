...
class LinkedInService:
    ...
    def __init__(self):
        ...
        self.redirect_uri = (
            ...
            or f"{settings.BACKEND_URL}/api/v1/linkedin/auth/callback"
        )
    ...
    async def get_user_profile(self, access_token: str) -> Dict[str, Any]:
        ...
        async with httpx.AsyncClient() as client:
            ...
            profile_response = await client.get(
                f"{self.base_url}/people/~", headers=headers
            )
    ...
    async def get_organizations(self, access_token: str) -> List[Dict[str, Any]]:
        ...
        async with httpx.AsyncClient() as client:
            response = await client.get(
                f"{self.base_url}/organizationAcls?q=roleAssignee", headers=headers
            )
            ...
            for element in data.get("elements", []):
                ...
                if org_id:
                    ...
                    org_response = await client.get(
                        f"{self.base_url}/organizations/{org_id}", headers=headers
                    )
    ...
    async def post_job(
        self, access_token: str, organization_id: str, job_data: Dict[str, Any]
    ) -> Dict[str, Any]:
        """
        ...
        async with httpx.AsyncClient() as client:
            response = await client.post(
                f"{self.base_url}/simpleJobPostings", headers=headers, json=linkedin_job
            )
    ...
    async def update_job(
        self, access_token: str, job_id: str, updates: Dict[str, Any]
    ) -> Dict[str, Any]:
        """
    ...
    async def delete_job(self, access_token: str, job_id: str) -> Dict[str, Any]:
        ...
        async with httpx.AsyncClient() as client:
            response = await client.delete(
                f"{self.base_url}/simpleJobPostings/{job_id}", headers=headers
            )
    ...
    def _format_job_for_linkedin(
        self, job_data: Dict[str, Any], organization_id: str
    ) -> Dict[str, Any]:
        """
        ...
        experience_mapping = {"entry": "1", "mid": "2", "senior": "3", "executive": "4"}

        linkedin_job = {
            "companyApplyUrl": job_data.get("apply_url", ""),
            "description": job_data.get("description", ""),
            "employmentStatus": job_type_mapping.get(
                job_data.get("job_type", "full_time"), "F"
            ),
            ...
            "workplaceTypes": (
                ["remote"] if job_data.get("remote_allowed") else ["onsite"]
            ),
        }
    ...
    def _get_job_function_code(self, job_title: str) -> str:
        ...
        if any(
            term in title_lower
            for term in ["engineer", "developer", "programmer", "architect"]
        ):
            return "eng"
        elif any(
            term in title_lower for term in ["sales", "account", "business development"]
        ):
            return "sal"
        elif any(
            term in title_lower
            for term in ["marketing", "brand", "content", "social media"]
        ):
            return "mkt"
        elif any(
            term in title_lower
            for term in ["hr", "human resources", "recruiter", "talent"]
        ):
            return "hum"
        elif any(
            term in title_lower
            for term in ["finance", "accounting", "controller", "analyst"]
        ):
            return "fin"
        elif any(
            term in title_lower for term in ["operations", "logistics", "supply chain"]
        ):
            return "ops"
            ...
            term in title_lower
            for term in ["legal", "counsel", "attorney", "compliance"]
        ):
            return "leg"
        elif any(
            term in title_lower for term in ["consulting", "strategy", "advisory"]
        ):
            return "con"
        else:
            return "oth"  # Other
