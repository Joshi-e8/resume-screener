...
class CandidateService:
    ...
    async def get_candidates(
        ...
        candidates = await Candidate.find(query).skip(skip).limit(limit).sort("-created_at").to_list()
        
        return candidates, total
    
    async def update_candidate(self, candidate_id: str, candidate_data: CandidateUpdate) -> Optional[Candidate]:
        """
    ...
    async def update_status(self, candidate_id: str, status: CandidateStatus) -> Optional[Candidate]:
        """
    ...
    async def calculate_job_match_score(self, candidate_id: str, job_id: str) -> Optional[JobMatchScore]:
        """
        ...
        if job_skills:
            skill_match = len(candidate_skills.intersection(job_skills)) / len(job_skills)
            base_score += skill_match * 30
        
        # Experience level matching
        if candidate.years_of_experience:
            if job.experience_level.value == "entry" and candidate.years_of_experience <= 2:
                base_score += 10
            elif job.experience_level.value == "mid" and 2 <= candidate.years_of_experience <= 5:
                base_score += 15
            elif job.experience_level.value == "senior" and candidate.years_of_experience >= 5:
                base_score += 15
        ...
        match_score = JobMatchScore(
            ...
            match_details={
                ...
                "location_match": candidate.location == job.location if candidate.location and job.location else False
            }
        )
        ...
        candidate.job_match_scores = [
            score for score in candidate.job_match_scores 
    ...
    async def find_job_matches(self, candidate_id: str, limit: int = 10) -> List[Dict]:
        ...
        for job in jobs:
            ...
            match_score = await self.calculate_job_match_score(candidate_id, str(job.id))
            if match_score:
                matches.append({
                    "job_id": str(job.id),
                    "job_title": job.title,
    ...
    async def get_candidate_stats(self, user_id: str) -> Dict:
        ...
        new_candidates = await Candidate.find({"user_id": user_id, "status": CandidateStatus.NEW}).count()
        shortlisted = await Candidate.find({"user_id": user_id, "status": CandidateStatus.SHORTLISTED}).count()
        interviewed = await Candidate.find({"user_id": user_id, "status": CandidateStatus.INTERVIEWED}).count()
        hired = await Candidate.find({"user_id": user_id, "status": CandidateStatus.HIRED}).count()
