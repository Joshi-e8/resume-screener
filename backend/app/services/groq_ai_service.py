...
class GroqAIService:
    ...
    def __init__(self):
        ...
        self.client = Groq(
            api_key=getattr(settings, 'GROQ_API_KEY', 'placeholder-key')
        )
        self.model = "llama3-8b-8192"  # GROQ's fast model
    
    async def analyze_resume(self, resume_text: str, parsed_data: Dict) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            if not hasattr(settings, 'GROQ_API_KEY') or settings.GROQ_API_KEY == 'placeholder-key':
    ...
    def _create_resume_analysis_prompt(self, resume_text: str, parsed_data: Dict) -> str:
    ...
    def _mock_resume_analysis(self, parsed_data: Dict) -> Dict[str, Any]:
        ...
        return {
            "strengths": [
                f"Strong technical skills in {', '.join(skills[:3])}" if skills else "Technical background",
                f"{len(experience)} years of relevant experience" if experience else "Professional experience",
            ...
            "key_skills": skills[:5] if skills else ["Communication", "Problem Solving"],
            "career_progression": "Steady growth trajectory" if len(experience) > 1 else "Early career",
            "overall_quality": 8.5,
            "recommendations": [
    ...
    def _mock_job_match_score(self, parsed_resume: Dict, job_requirements: List[str]) -> Dict[str, Any]:
        ...
        skill_match_score = min((skill_overlap / len(required_skills) * 100) if required_skills else 50, 100)
        
        overall_score = (skill_match_score + 75 + 80 + 70) / 4  # Average with mock scores
    ...
    def _mock_skill_extraction(self, text: str) -> List[str]:
        ...
        common_skills = [
            ...
            'AWS', 'Docker', 'MongoDB', 'PostgreSQL', 'REST API',
